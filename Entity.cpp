...
XMFLOAT2 Entity::CalcAttractionAcc(Entity* entity)
{
    XMFLOAT3 entityPos = entity->GetPositionFloat3();

    float xDistance = _pos.x - entityPos.x;
    float yDistance = _pos.y - entityPos.y;
    float distance = sqrtf(xDistance * xDistance + yDistance * yDistance + (0.1f * 0.1f));

    float xAccelleration(0.f), yAccelleration(0.f);
    if (distance > 20.0f)
    {
        ...
        xAccelleration = xNormalized * attractionForce;
        yAccelleration = yNormalized * attractionForce;
    ...
    {
        xAccelleration = yAccelleration = 0.0f;
    }
 
    return XMFLOAT2(xAccelleration, yAccelleration);
}
